...
def test_unauthenticated_user_cant_create_exercise_types(
...
def test_authenticated_user_can_create_exercise_types(
    client: TestClient,
    ...
    with session_factory() as session:
        result = session.execute(
            delete(ExerciseType).where(ExerciseType.id == UUID(resource["id"]))
        )
        session.commit()
