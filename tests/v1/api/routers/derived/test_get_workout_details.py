...
def test_unauthenticated_user_cant_read(
    ...
    # Try with no creds.
    response = client.get(f"{ROUTE}/{workout.id}")
    ...
    response = client.get(f"{ROUTE}/{workout.id}", headers=bad_user_auth)
    # Make sure we get a 401 and no data comes back.
    assert response.status_code == 401
...
def test_user_cant_read_others_workouts(
    ...
    workout, exercise_type = primary_user_workout_and_exercise_type
    response = client.get(f"{ROUTE}/{workout.id}", headers=secondary_test_user.auth)
    assert response.status_code == 404


def test_invalid_id_returns_404(client: TestClient, primary_test_user: UserWithAuth):
    imaginary_id = uuid4()
    response = client.get(f"{ROUTE}/{imaginary_id}", headers=primary_test_user.auth)
    assert response.status_code == 404
...
def test_returns_correct_workout_details(
    ...
    response = client.get(f"{ROUTE}/{workout.id}", headers=primary_test_user.auth)
    assert response.status_code == 200
    payload = response.json()
    assert payload.keys() == {"workout", "exercises"}
    payload_workout = payload["workout"]
    ...
    payload_exercises = payload["exercises"]
...
def test_returns_correct_workout_details_when_there_are_no_exercises(
    ...
    assert len(payload["exercises"]) == 0
